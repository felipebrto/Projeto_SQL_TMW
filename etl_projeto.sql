...
--saber se a pessoa acompanha(transacao) nos dias 7, 14, 28 e 56 
--(vida é valor total)


---transformamos julian day em diffdate para ficar menor a query
-- SELECT IdCliente,
--        count(*) As Qtdetransacoes,
--        count(CASE WHEN julianday ('now') - julianday(substr(DtCriacao,1,10)) <= 7 THEN IdTransacao END) As qtdtransacoesD7,
--        count(CASE WHEN julianday ('now') - julianday(substr(DtCriacao,1,10)) <= 14 THEN IdTransacao END) As qtdtransacoesD14
-- FROM transacoes
-- GROUP BY IdCliente

--min é o ultimo dia que a pessoa acompanhou o conteudo 
